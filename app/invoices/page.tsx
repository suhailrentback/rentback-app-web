...
import { SortHeader, type SortDir, type SortKey } from '@/components/SortControls';
          ...
          >
            <tr>
              <th className="text-left p-3 font-medium w-36" aria-sort={ariaFor('number')}>
                <SortHeader label="Number" field="number" activeSort={sort} dir={dir}
                  status={filter === 'all' ? undefined : filter} q={q} page={page} />
              </th>
              <th className="text-left p-3 font-medium w-40" aria-sort={ariaFor('created_at')}>
                <SortHeader label="Created" field="created_at" activeSort={sort} dir={dir}
                  status={filter === 'all' ? undefined : filter} q={q} page={page} />
              </th>
              <th className="text-left p-3 font-medium w-40" aria-sort={ariaFor('due_at')}>
                <SortHeader label="Due" field="due_at" activeSort={sort} dir={dir}
                  status={filter === 'all' ? undefined : filter} q={q} page={page} />
              </th>
              ...
              <th className="text-right p-3 font-medium w-36" aria-sort={ariaFor('total')}>
                <SortHeader label="Total" field="total" activeSort={sort} dir={dir}
                  status={filter === 'all' ? undefined : filter} q={q} page={page} />
              </th>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((inv) => (
                <tr
                  key={inv.id}
                  className={clsx(
                    'relative border-t border-black/5 dark:border-white/10 transition-colors',
