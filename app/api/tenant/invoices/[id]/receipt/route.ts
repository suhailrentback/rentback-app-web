...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.error("Invoice zod parse failed (receipt):", e);
  ...
  doc.moveDown(0.25);
  doc.fontSize(10).fillColor("#6B7280").text("Payment Receipt", { continued: false });
  ...
  doc.moveDown(0.5);
  doc.fontSize(12).fillColor("#111827").text(`Amount Received: ${money(amt, currency)}`);
