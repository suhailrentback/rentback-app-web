...
export async function POST(req: Request) {
  ...
  // 1) Load payment + invoice (scoped via staff/admin; RLS allows)
  ...
  // 2) Confirm payment + mark invoice PAID (idempotent-ish)
  // Do in a single RPC-ish transaction via sequential calls (simple and safe for now).
  const nowIso = new Date().toISOString();

  // 2a) Update payment -> confirmed (if not already)
  ...
  // 2b) Update invoice -> paid
  const { error: upInvErr } = await sb
    .from("invoices")
    .update({ status: "paid" })
    .eq("id", invoiceId);
  if (upInvErr) return NextResponse.json({ error: "update_invoice_failed", detail: upInvErr.message }, { status: 500 });

  // 2c) Insert receipt (ignore conflict if already exists)
  const { data: receiptIns, error: rcptErr } = await sb
    .from("receipts")
    .insert({
  ...
  if (rcptErr && rcptErr.code !== "23505") {
    // not a duplicate unique violation
    return NextResponse.json({ error: "insert_receipt_failed", detail: rcptErr.message }, { status: 500 });
  }

  // 3) Email the tenant (no-op if no key or email missing)
  //    Look up tenant profile via invoice.tenant_id -> profiles
  let tenantEmail = "";
  let tenantName = "";
  if (payment.invoice.tenant_id) {
    const { data: tenantProf } = await sb
      .from("profiles")
      .select("email, full_name")
      .eq("user_id", payment.invoice.tenant_id)
      .maybeSingle();
  ...
  if (tenantEmail) {
    const subject = `Receipt: ${payment.invoice.number} — ${formatMoney(payment.amount_cents, payment.currency)}`;
    const html = `
      <p>Hi ${tenantName || "there"},</p>
      <p>Your payment <strong>${formatMoney(payment.amount_cents, payment.currency)}</strong> for invoice <strong>${payment.invoice.number}</strong> has been confirmed.</p>
    ...
    // Try to attach a PDF (optional). If it fails, send without attachment.
    let attachments: { filename: string; content: string }[] | undefined = undefined;
    try {
      const pdfBuf = await buildReceiptPdf({
        tenantName,
        tenantEmail,
        invoiceNumber: String(payment.invoice.number || ""),
      ...
      attachments = [
        {
          filename: `receipt-${payment.invoice.number || invoiceId}.pdf`,
          content: pdfBuf.toString("base64"),
        },
      ];
    } catch {
      // swallow; we'll just send without attachment
    }
  ...
  // 4) Redirect back to Admin /payments (PRG pattern)
